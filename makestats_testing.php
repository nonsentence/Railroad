...
while ($i < $argc)
{
	...
	if (substr_count($argv[$1], "71") > 0)
		$version = "71";

	if (substr_count($argv[$i], "11") > 0)
		$version = "11";
	$i++;
}
...
$log_path = "/opt/gameserver/server/half-life/cstrk" . $version . "/logs";
$log_path_old = "/opt/gameserver/server/half-life/cstrk" . $version . "/logs/old";

$no_players = "Upload, Upload By t4r1k, Anonymous, Blaster, Player, Freshmeat";

$num_files = 0;
$num_lines = 0;
$script_path = "/root/scripts/csstats/";
...
function write_data()
{
	...
	while ($i < count($players['name']))
	{
		...
		if ($ct_deaths == 0 OR $ct_deaths + $ct_kills < 20)
			$ratio = 0;
		else
	...
	while ($i < count($weapons['id']))
	{
		...
		$query = "UPDATE $sqltable_weapons SET ct_kills='$ct_kills' WHERE id='$id'";
		mysql_query($query);
		$i++;
	}
