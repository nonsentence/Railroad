...
$log_path = "/usr/local/games/hlb/hlds_l/cstrk" . $version . "/logs";
$log_path_old = "/usr/local/games/hlb/hlds_l/cstrk" . $version . "/logs/old";
...
function analyse_file($file_name)
{
    ...
    {    
        ...
        if (substr_count($line, " killed ") == 1)
        {
            ...
            } else if (substr_count($line, "[CT]") == 1)
            { 
                //$player1 killed $player2 who is a CT with $weapon
                $player1 = substr($line_pieces[1], 0, strrpos($line_pieces[1], "<"));

                $player2_raw = substr($line_pieces[3], 5);
                $player2 = substr($player2_raw, 0, strrpos($player2_raw, "<"));
		...
		//		print($player2 . "\n");
            ...
            } else if (substr_count($line, "[TERRORIST]") == 1)
            {
                //$player1 killed $player2 who is a TERRORIST with $weapon
				$player1 = substr($line_pieces[1], 0, strrpos($line_pieces[1], "<"));

                $player2_raw = substr($line_pieces[3], 12);
                $player2 = substr($player2_raw, 0, strrpos($player2_raw, "<"));
